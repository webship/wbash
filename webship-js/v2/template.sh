...
## Remove the old cucumber.js file.
if [[ -f "${local_project_path}/cucumber.js" ]]; then
  rm ${local_project_path}/cucumber.js;
fi

## Append webship-js test script command to package.json file.
if [ -f "${local_project_path}/package.json" ]; then

  npm pkg set scripts.test="cucumber-js --config cucumber.js; node generate-reports.js;"
fi
...
## Add package.json if not exist file.
if [ ! -f "${local_project_path}/package.json" ]; then
...
## Place cucumber.js file in its target path.
mv ${local_project_path}/${version}/cucumber.js ${local_project_path}/cucumber.js;
